...
;(function($) {
	$.fn.rwdImageMaps = function() {
        ...
        copyimg[0].onload = function() {
          //$('<img />').load(function() {
          //先尝试使用<img width='1024' height='768' />获取原图的属性值(初始值由开发人员指定，一旦指定后，属性值作为初始值在响应式调整中不会改变)
          //使用native方法getAttribute('width')或者jQuery方法attr('width')可以访问该属性值
          //使用native属性width或者jQuery方法width()获取图片的真实尺寸值(动态的)
          var attrW = 'width',
          attrH = 'height',
          w = $that.attr(attrW),
          h = $that.attr(attrH);
